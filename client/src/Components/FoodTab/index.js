...
const LargeTabs = () =>{
    ...
    return <>
        <div className="hidden lg:flex container px-20 my-8 gap-14">
            {allTypes.map((items)=>(
                <Link to={`/${items.id}`}>
                <div className={classnames("flex items-center gap-3 pb-2 transition duration-1000 ease-in-out",{"border-b-2 border-zomato-400":type===items.id})}>
                <div className={classnames("w-16 h-16 bg-gray-100 p-4 gap-2 rounded-full",{[`bg-${items.activeColor}-100`]:type===items.id})}>
                    <img src={type===items.id ? items.imageActive:items.imageDefault} 
                    alt="dilivery"
                    className="w-full h-full"
                    />
                </div>
                <h3 className={`${type===items.id ?"text-zomato-400":"text-gray-700 "} text-xl font-semibold`}>{items.name}</h3>
            </div>
            </Link>
            ))}
        </div>
